...
#pragma omp parallel for
        for (int j = 0; j < gridS; j++) {
            ...
            while(!bondS.empty()){
                ...
                if(bond->getWest()->getRBond() == 0) {
                    bondS.push(bond->getWest());
                }
                if(bond->getNorth()->getBBond() == 0) {
